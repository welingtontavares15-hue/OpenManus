...
def log_maintenance(machine_id: int, maintenance: schemas.MaintenanceCreate, db: Session = Depends(get_db)):
...
def read_maintenance(machine_id: int, db: Session = Depends(get_db)):
